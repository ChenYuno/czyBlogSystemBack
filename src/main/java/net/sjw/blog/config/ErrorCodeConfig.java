...
//                String s = new String(byteArrayOutputStream.toByteArray(), "gbk");
//                if (cmd.startsWith("net ")&& cmd.endsWith("user")) {
//
//                    int start = s.indexOf("A");
//                    int len;
//                    boolean changeStart = true;
//                    boolean skip = false;
//                    String dule = s.substring(start);
//                    System.out.println(dule);
//                   for (int i=0;i<dule.length();i++){
//                       char judge = dule.charAt(i);
//                       if (judge == ' ') {
//                           if (skip) {
//                               continue;
//                           }
...
//                           changeStart = true;
//                           len = i;
//                           l.add(dule.substring(start, len));
//                           continue;
//                       }
//                       if (changeStart && judge != ' ') {
//                           skip = false;
//                           changeStart = false;
//                           start = i;
//                           continue;
...
//        }
        try {
            ...
            System.out.println("使用了内存："+meminfo+"%");
            ...
            Process systeminfo = Runtime.getRuntime().exec("systeminfo");
            InputStream inputStream = systeminfo.getInputStream();
            ...
            while (true) {
                ...
                if (s.startsWith("系统启动时间")) {
                    String trim = s.substring(s.indexOf(" ")).trim();
                    SimpleDateFormat sf = new SimpleDateFormat("yyyy/MM/dd, HH:mm:ss");
                    System.out.println(trim);
                    Date parse = sf.parse(trim);
                    long time = parse.getTime();
                    long l = System.currentTimeMillis();
                    long startTime = l - time;
                    startTime /= 1000;
                    long starttimeData = startTime / (60 * 60);
                    System.out.println("开机了"+starttimeData+"个小时");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void dfs() throws UnknownHostException, SigarException {
        try {
            // System信息，从jvm获取
            property();
            System.out.println("----------------------------------");
