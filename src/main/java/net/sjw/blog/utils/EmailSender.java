...
public class EmailSender {
    ...
    //TODO:Complete your email configuration.
    private EmailSender() {
        EmailSender.config(EmailSender.SMTP_QQ(false), "你的邮箱", "你邮箱的smtp密码");
    }
    ...
    public static void sendRegisterVerifyCode(String code,String address) throws Exception {
        EmailSender.subject("czy博客系统验证码")
                .from("czyBlogSystem")
                .text("您的验证码是： "+code+" 有效期为10分钟，非本人操作请忽略此邮箱.")
                .to(address)
                .send();
    }

    public static void sendCommentNotify(String notifyContent,String address) throws Exception {
        EmailSender.subject("czy博客系统-评论通知")
                .from("czyBlogSystem")
//                .html()
                .text(notifyContent)
                .to(address)
